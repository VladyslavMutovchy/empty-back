...
// обновление данных
  adminListApi.route('/api/v1/admin-list/update/:id').post(async (req, res) => {
    try {
      const result = await AdminListAction.update(req.body);
      res.end(JSON.stringify(result));
    } catch (e) {
      console.log('Error updating data', e);
      res.status(500).end('Error updating data');
    }
  });
  ...
  adminListApi.route('/api/v1/admin-list/deleteAll').delete(async (req, res) => {
    try {
      ...
      res.end(JSON.stringify(result)); // Возвращаем подтверждение удаления
    } catch (e) {
      console.log('Error deleting all data', e);
      res.status(500).end('Error deleting all data');
    }
  });
