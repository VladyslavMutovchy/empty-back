...
async sendMessage(chat_id, message, from_user_id) {
  const sendMessageQuery = `
    INSERT INTO messages (in_chat_id, from_user_id, message, created_at)
    VALUES (?, ?, ?, NOW())
  `;

  await global.dbConnection.runQuery(sendMessageQuery, [chat_id, from_user_id, message]);
  return { success: true };
}
