...
class MongoAdminListAction {
  ...
  async getAll( start, limit, search, filter_by, sort_by, sort_direction ) {
    ...
    // Агрегатный запрос
    const datum = await global.mongoConnection.aggregate('admin-list', mongoParams);
  ...
  async deleteAll() {
    await global.mongoConnection.delete('admin-list', {});
    await global.mongoConnection.delete('admin-list-skills', {});
  }
}
